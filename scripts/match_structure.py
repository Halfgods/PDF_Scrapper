...
def extract_courses(sem_text):
    ...
    while i < len(lines):
        ...
        if course_code_pattern.match(line):
            ...
            # Look ahead up to the next three lines for possible course names
            j = i + 1
            while j < len(lines) and j < i + 4:
                candidate = lines[j].strip()
                if candidate and not course_code_pattern.match(candidate) and not candidate.isdigit() and not any(skip_term in candidate for skip_term in skip_terms):
                    candidates.append(candidate)
                j += 1

            # Choose the longest candidate as the course name (if available)
            if candidates:
                course_name = max(candidates, key=len)
            else:
                course_name = ""
